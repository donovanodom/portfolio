    ...
    <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 grid-cols-1">
      {miniApps.length && miniApps.map((miniApp: MiniApp) => (
        <Link href={'/mini-apps/' + miniApp.id}>
          <div key={miniApp._id} className="text-black pointer [&_pre]:whitespace-pre-wrap overflow-hidden max-h-[600px] md:max-h-[400px] p-2 lg:p-0 [mask-image:linear-gradient(0deg,transparent_0%,#000_40%,#000_80%)]">
